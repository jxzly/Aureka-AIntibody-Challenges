...
# from datasets.msa_featurizer import MSAFeaturizer
# from DistilledBinding.datasets.data.parser import DistillationMMCIFParser, MMCIFParser
from datasets.tokenizer import AtomArrayTokenizer, TokenArray
from utils.cropping import CropData
# from DistilledBinding.utils.file_io import load_gzip_pickle
...
class DataPipeline(object):
    ...
    def crop(
        ...
        # msa_featurizer: Optional[MSAFeaturizer],
        # template_featurizer: None,
        method_weights: list[float] = [1.0, 0.0, 0.0],
        ...
        # )
        selected_indices = crop.get_crop_indices(
            crop_method=crop_method
        )
        # Prepare msa
        # msa_features = DataPipeline.get_msa_raw_features(
        ...
        if crop_method == "ContiguousCropping":
            ...
            resovled_atom_num = len(cropped_atom_array)
